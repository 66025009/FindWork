...
const userData = computed(() => {
  ...
  return {
    profileImage: currentUser?.profileImage || '/profileImage.jpg',
...
onMounted(async () => {
  ...
  // โหลดความคิดเห็นใหม่หลังจากการอัปเดตข้อมูล
  await Promise.all(posts.value.map(post => postStore.fetchComments(post.id)));
